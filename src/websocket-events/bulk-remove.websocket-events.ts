...
export default class BulkRemoveWebsocketEvents {
  static Started = class {
    ...
    static EventDataEntity = class extends createZodDto(this.EventDataSchema) implements WebsocketEventTostablePort {
      ...
      getTitle(attempt: number): string {
        let title = `A remoção em massa de "${mapExportResourceToName(this.resource)}" foi iniciada...`;
        if (attempt > 1) {
  ...
  static Progress = class {
    ...
    static EventDataEntity = class extends createZodDto(this.EventDataSchema) implements WebsocketEventTostablePort {
      ...
      getTitle(attempt: number): string {
        let title = `A remoção em massa de "${mapExportResourceToName(this.resource)}" está em progresso...`;
        if (attempt > 1) {
          title += ` (tentativa ${attempt})`;
        }
        return title;
  ...
  static Finished = class {
    ...
    static EventDataEntity = class extends createZodDto(this.EventDataSchema) implements WebsocketEventTostablePort {
      ...
      getTitle(attempt: number): string {
        let title = `A remoção em massa de "${mapExportResourceToName(this.resource)}" foi finalizada.`;
        if (attempt > 1) {
          title += ` (tentativa ${attempt})`;
        }
        return title;
