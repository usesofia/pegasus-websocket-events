...
exports.BulkOperationForBankTransactionsWebsocketEvents = void 0;
const bulk_async_job_result_status_enum_1 = require("../enums/bulk-async-job-result-status.enum");
const bulk_bank_transactions_operation_enum_1 = require("../enums/bulk-bank-transactions-operation.enum");
...
const StartedSchema = zod_1.z.object({
    ...
    operation: zod_1.z.nativeEnum(bulk_bank_transactions_operation_enum_1.BulkBankTransactionsOperation),
});
class StartedEventDataEntity extends zod_class_1.Z.class(StartedSchema.shape) {
    ...
    getTitle(attempt) {
        let title = `A operação em massa de "${(0, bulk_bank_transactions_operation_enum_1.mapBulkBankTransactionsOperationToName)(this.operation)}" as transações bancárias foi iniciada...`;
        if (attempt > 1) {
...
class ProgressEventDataEntity extends zod_class_1.Z.class(ProgressSchema.shape) {
    ...
    getTitle(attempt) {
        let title = `A operação em massa de "${(0, bulk_bank_transactions_operation_enum_1.mapBulkBankTransactionsOperationToName)(this.operation)}" as transações bancárias está em progresso...`;
        if (attempt > 1) {
            title += ` (tentativa ${attempt})`;
        }
        return title;
...
class FinishedEventDataEntity extends zod_class_1.Z.class(FinishedSchema.shape) {
    ...
    getTitle(attempt) {
        let title = `A operação em massa de "${(0, bulk_bank_transactions_operation_enum_1.mapBulkBankTransactionsOperationToName)(this.operation)}" as transações bancárias foi finalizada.`;
        if (attempt > 1) {
            title += ` (tentativa ${attempt})`;
        }
        return title;
...
exports.BulkOperationForBankTransactionsWebsocketEvents = {
    Started: {
...
//# sourceMappingURL=bulk-operation-for-bank-transactions.websocket-events.js.map