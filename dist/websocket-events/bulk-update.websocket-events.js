...
exports.BulkUpdateWebsocketEvents = void 0;
...
class StartedEventDataEntity extends zod_class_1.Z.class(StartedSchema.shape) {
    ...
    getTitle(attempt) {
        let title = `A atualização em massa de "${(0, resource_enum_1.mapResourceToName)(this.resource)}" foi iniciada...`;
        if (attempt > 1) {
...
class ProgressEventDataEntity extends zod_class_1.Z.class(ProgressSchema.shape) {
    ...
    getTitle(attempt) {
        let title = `A atualização em massa de "${(0, resource_enum_1.mapResourceToName)(this.resource)}" está em progresso...`;
        if (attempt > 1) {
            title += ` (tentativa ${attempt})`;
        }
        return title;
...
class FinishedEventDataEntity extends zod_class_1.Z.class(FinishedSchema.shape) {
    ...
    getTitle(attempt) {
        let title = `A atualização em massa de "${(0, resource_enum_1.mapResourceToName)(this.resource)}" foi finalizada.`;
        if (attempt > 1) {
            title += ` (tentativa ${attempt})`;
        }
        return title;
    ...
    getDescription() {
        switch (this.resultStatus) {
            ...
            case bulk_async_job_result_status_enum_1.BulkAsyncJobExecutionResultStatus.PROCESSED_ALL_ITEMS_AND_ALL_SUCCESSED:
                return `Dos ${this.nTotalItems} registros previstos, todos foram atualizados com sucesso.`;
            case bulk_async_job_result_status_enum_1.BulkAsyncJobExecutionResultStatus.PROCESSED_ALL_ITEMS_AND_ALL_FAILED:
                return `Dos ${this.nTotalItems} registros previstos, todas as atualizações falharam.`;
            case bulk_async_job_result_status_enum_1.BulkAsyncJobExecutionResultStatus.PROCESSED_PART_OF_THE_ITEMS_AND_ALL_OF_THEM_SUCCESSED:
                return `Dos ${this.nTotalItems} registros previstos, ${this.nSuccessItems} foram atualizados com sucesso.`;
            case bulk_async_job_result_status_enum_1.BulkAsyncJobExecutionResultStatus.PROCESSED_PART_OF_THE_ITEMS_AND_ALL_OF_THEM_FAILED:
                return `Dos ${this.nTotalItems} registros previstos, ${this.nFailedItems} atualizações falharam.`;
            case bulk_async_job_result_status_enum_1.BulkAsyncJobExecutionResultStatus.PROCESSED_ALL_ITEMS_AND_SOME_SUCCESSED_AND_SOME_FAILED:
                return `Dos ${this.nTotalItems} registros previstos, ${this.nSuccessItems} foram atualizados com sucesso e ${this.nFailedItems} falharam.`;
...
exports.BulkUpdateWebsocketEvents = {
    Started: {
...
//# sourceMappingURL=bulk-update.websocket-events.js.map